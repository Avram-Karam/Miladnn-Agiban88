...
class DataService {
    ...
    static async sendToGAS(payload) {
        ...
        const publicActions = new Set([
            'saveIndividualScore','addFeedback','ping',
    ...
    static _mergeGASIntoConference(confData, gasItems) {
        ...
        // SiteConfig هي المرجع النهائي لتقسيم المجموعات الذي يعدله الأدمن.
        // هذا يمنع أسماء قديمة من Google Sheets من الرجوع للمجموعة بعد حذفها من لوحة الأدمن.
        const memberToGroup = new Map();
        const configuredMembers = new Set();
        if (Array.isArray(confData.groups)) {
            confData.groups.forEach(g => {
                (Array.isArray(g.members) ? g.members : []).forEach(member => {
                    const key = String(member).trim().toLowerCase();
                    if (!key) return;
                    memberToGroup.set(key, {name:String(g.name||'').trim(), id:String(g.id||'').trim()});
                    configuredMembers.add(key);
                });
            });
        }

        // ونفس الفكرة للتسكين: SiteConfig.rooms هو المرجع النهائي للغرف.
        const memberToRoom = new Map();
        if (Array.isArray(confData.rooms)) {
            confData.rooms.forEach(room => {
                const roomName = String(room.name || room.room || '').trim();
                const roomId = String(room.id || '').trim();
                (Array.isArray(room.persons) ? room.persons : []).forEach(person => {
                    const key = String(person).trim().toLowerCase();
                    if (key) memberToRoom.set(key, {name: roomName.replace(/^غرفة\s*/,'') || roomName, id: roomId});
                });
            });
        }
        ...
        gasItems.forEach(item => {
            ...
            // لو الاسم موجود في SiteConfig groups نأخذ المجموعة منه.
            // ولو لم يعد موجوداً في أي مجموعة، لا نعيد له المجموعة القديمة من الشيت.
            const rawGroup = (memberGroup?.name || (configuredMembers.size && !configuredMembers.has(key) ? '' : (item.group || base.group || base.groupId || ''))).trim();
            ...
            const configuredRoom = memberToRoom.get(key);
            if (configuredRoom) {
                p.room = String(configuredRoom.name || '').replace(/^غرفة\s*/,'');
                p.roomId = configuredRoom.id || (p.room ? 'r' + p.room : null);
            } else if (memberToRoom.size) {
                // الاسم أزيل من كل الغرف في لوحة الأدمن.
                p.room = '';
                p.roomId = null;
            } else if (typeof item.room !== 'undefined') {
