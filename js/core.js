...
async function loadPartials() {
    ...
    refreshNavbarScoreboard();
}

async function refreshNavbarScoreboard() {
    const board = document.getElementById('top-gradeboard');
    const attendanceValue = document.getElementById('top-attendance-value');
    const pamphletValue = document.getElementById('top-pamphlet-value');
    if (!board || !attendanceValue || !pamphletValue) return;

    const raw = localStorage.getItem('yc_participant_profile_v1');
    if (!raw) {
        attendanceValue.textContent = '— / 120';
        pamphletValue.textContent = '— / 50';
        return;
    }
    ...
    try {
        const prefix = location.pathname.includes('/pages/') ? '../' : '';
        const [structureRes, scorebook] = await Promise.all([
            fetch(prefix + 'assets/data/conference-data.json?grades=' + Math.floor(Date.now()/30000)),
            window.DataService?.getScorebook ? DataService.getScorebook() : Promise.reject(new Error('DataService unavailable'))
        ]);
        const structure = await structureRes.json();
        const g = (structure.groups || []).find(x => String(x.name).trim() === group);
        const members = Array.isArray(g?.members) ? g.members : [];
        const norm = s => String(s||'').trim().toLowerCase().replace(/[إأآ]/g,'ا').replace(/ى/g,'ي').replace(/ة/g,'ه').replace(/ـ/g,'').replace(/\s+/g,' ');
        const same = (a,b) => norm(a) === norm(b);
        const indiv = scorebook?.individualScores || [];
        const avg = (cat,max) => {
            if (!members.length) return 0;
            let total = 0;
            members.forEach(name => {
                const rows = indiv.filter(r => same(r.group, group) && same(r.name, name) && String(r.category) === cat);
                const row = rows[rows.length - 1];
                total += Math.min(max, Math.max(0, Number(row?.score || 0)));
            });
            return total / members.length;
        };
        let attendance = 0;
        for (let i = 1; i <= 8; i++) attendance += avg('attendance' + i, 15);
        const pamphlet = avg('pamphlet', 50);
        attendanceValue.textContent = `${attendance.toFixed(1).replace(/\.0$/,'')} / 120`;
        pamphletValue.textContent = `${pamphlet.toFixed(1).replace(/\.0$/,'')} / 50`;
    } catch(e) {
        // تظل آخر قيمة ظاهرة؛ ولا نُخرج المستخدم من صفحته عند فشل التحديث.
    } finally {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('focus', () => { refreshNavbarScoreboard(); });
    window.addEventListener('pageshow', () => { refreshNavbarScoreboard(); });
