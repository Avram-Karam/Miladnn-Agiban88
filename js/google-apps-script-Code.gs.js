...
function doPost(e) {
  try {
    ...
    if (action === 'adminDeleteScore') {
      return handleAdminDeleteScore(sheet, body);
    }
...
function handleAdminSetScore(sheet, body) {
  ...
  if(type==='individual') {
    const name=String(body.name||'').trim(), group=String(body.group||'').trim(), category=String(body.category||'').trim();
    const allowed={lecture1:15,lecture2:15,attendance1:15,attendance2:15,attendance3:15,attendance4:15,attendance5:15,attendance6:15,attendance7:15,attendance8:15,pamphlet:50};
    if(!name || !group || allowed[category]==null) return corsOutput({status:'error',message:'بيانات الدرجة الفردية ناقصة أو نوع الدرجة غير مسموح'});
    const max=allowed[category], score=Math.max(0,Math.min(max,Number(body.score||0)));
...
function handleAdminDeleteScore(sheet, body) {
  const name=String(body.name||'').trim();
  const group=String(body.group||'').trim();
  const category=String(body.category||'').trim();
  if(!name || !group || !category) return corsOutput({status:'error',message:'بيانات حذف الدرجة ناقصة'});
  const ss=sheet.getParent();
  ...
  for(let i=rows.length-1;i>=1;i--){
    if(String(rows[i][0]||'').trim().toLowerCase()===name.toLowerCase() &&
       ...
       String(rows[i][2]||'').trim()===category){
      is.deleteRow(i+1);
      return corsOutput({status:'success',message:'تم حذف الدرجة بنجاح.',name,group,category});
    }
  }
  return corsOutput({status:'success',message:'لا توجد درجة مسجلة لهذا الشخص في هذا البند.',name,group,category});
}
